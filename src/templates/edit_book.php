<?php require_once '../src/templates/_header.php'; ?>

<div class="add-book-container">
    <h1>Modifier les informations</h1>
    
    <form action="index.php?action=editBookPost&id=<?= $book->getId() ?>" method="post" enctype="multipart/form-data">
        
        <p>
            <label for="title">Titre du livre</label><br>
            <input type="text" name="title" id="title" value="<?= htmlspecialchars($book->getTitle()) ?>" required>
        </p>

        <p>
            <label for="author">Auteur</label><br>
            <input type="text" name="author" id="author" value="<?= htmlspecialchars($book->getAuthor()) ?>" required>
        </p>

        <p>
            <label for="description">Commentaire / Description</label><br>
            <textarea name="description" id="description" rows="5" required><?= htmlspecialchars($book->getDescription()) ?></textarea>
        </p>

        <p>
            <label for="available">Disponibilité</label><br>
            <select name="available" id="available">
                <option value="disponible" <?= $book->getDisponibilite() === 'disponible' ? 'selected' : '' ?>>Disponible</option>
                <option value="non disponible" <?= $book->getDisponibilite() === 'non disponible' ? 'selected' : '' ?>>Non disponible</option>
            </select>
        </p>

        <p>
            <label for="image">Photo du livre</label><br>
            <img src="/Mission_tomtroc/public/img/books/<?= htmlspecialchars($book->getImage()) ?>" alt="Actuelle" width="100"><br>
            <small>Laissez vide si vous ne voulez pas changer l'image</small><br>
            <input type="file" name="image" id="image" accept="image/png, image/jpeg">
        </p>

        <button type="submit" class="btn-save">Valider</button>
    </form>

    <p><a href="index.php?action=profile">Annuler</a></p>
</div>

<?php require_once '../src/templates/_footer.php'; ?>